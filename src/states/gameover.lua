...
function gameover:draw()
  ...
  draw.textCentered(locale.t("ui.gameover.header"), sw / 2, sh * 0.2, 30, { 1, 0.85, 0.75, a })
  ...
  if run then
    ...
    local lines = {
      locale.f("ui.gameover.fellIn", biomeName, run.depth),
      locale.f("ui.gameover.killsTime", run.kills, util.formatTime(run.time)),
      locale.f("ui.gameover.cinders", run.cindersEarned),
      locale.f("ui.gameover.seed", run.seed),
    }
    ...
    for _, owned in ipairs(run.boons) do
      ...
      if def then names[#names + 1] = boonsSys.name(def) end
    end
    if #names > 0 then
      local la = util.clamp(self.t * 1.4 - 1.4, 0, 1)
      draw.textCentered(locale.f("ui.gameover.carried", table.concat(names, " · ")),
        sw / 2, sh * 0.36 + 118, 10, { 1, 1, 1, 0.4 * la })
    end
  end

  local d = save.get()
  draw.textCentered(locale.f("ui.gameover.kilnHolds", d.cinders),
    sw / 2, sh * 0.66, 13, { 0.75, 0.88, 1, 0.85 })

  if self.t > 0.8 then
    draw.textCentered(locale.t("ui.gameover.footer"),
