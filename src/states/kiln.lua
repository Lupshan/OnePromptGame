...
function kiln:update(dt)
  ...
  elseif input.pressed("confirm") then
    ...
    if save.isUnlocked(item.id) then
      sfx.play("uiDeny")
      self.message = "Already burning."
    elseif save.spendCinders(item.cost) then
      save.unlock(item.id)
      sfx.play("unlock")
      self.message = item.name .. " kindled."
    else
      sfx.play("uiDeny")
      self.message = "Not enough cinders."
    end
...
function kiln:draw()
  ...
  draw.textCentered("THE KILN", sw / 2, sh * 0.06, 30, { 1, 0.9, 0.75, 1 })
  draw.textCentered("cinders buy new possibilities — never raw power", sw / 2, sh * 0.06 + 40, 11,
    { 1, 1, 1, 0.5 })
  ...
  for i, item in ipairs(self.items) do
    ...
    draw.text(item.name, x0 + 16, y + 7, 14, nameCol)
    draw.text(item.desc or "", x0 + 16, y + 26, 10, { 1, 1, 1, 0.5 })

    if owned then
      draw.text("lit", x0 + w - 50, y + 12, 12, { 1, 0.7, 0.4, 0.9 })
    else
  ...
  draw.textCentered("up/down browse · SPACE kindle · ESC back", sw / 2, sh - 48, 12, { 1, 1, 1, 0.5 })
